...
const TimeSelectionStep = ({ onBack, onContinue, onDateChange, onTimeChange, onTypeChange, selectedDate, selectedDentistId, selectedTime, selectedType }: TimeSelectionStepProps) => {
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {availableDates.map((date) => (
              <Button key={date} variant={selectedDate === date ? 'default' : 'outline'} onClick={() => handleDateSelect(date)} className="h-auto p-3">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                {availableTimeSlots.map((time) => {
                  ...
                  return (
                    <Button key={time} variant={selectedTime === time ? 'default' : 'outline'} onClick={() => !isBooked && onTimeChange(time)} size="sm" disabled={isBooked} className={isBooked ? 'opacity-50 cursor-not-allowed' : ''}>
